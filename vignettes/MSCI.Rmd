...
df1 <- aggregate(Market.Value~Date + Sector, data = minvol, FUN = sum, na.action = na.omit)
df1 <- mutate(df1, Sector = ifelse(Sector == "Telecommunications", "Telecommunication Services", Sector))
...
names(usa)[7] <- "Market.Value"
df2 <- aggregate(Market.Value~Date + Sector, data = usa, FUN = sum, na.action = na.omit)
df2 <- mutate(df2, Sector = ifelse(Sector == "Telecommunications", "Telecommunication Services", Sector))
...
# MSCI vs USA
ggplot() + geom_line(data = df1, mapping = aes(x = Date, y = Market.Value, color = "red")) +  geom_line(data = df2, mapping = aes(x = Date, y = Market.Value, color = "blue")) + scale_x_date(labels = date_format("%m-%Y"), date_breaks = "3 years") + facet_wrap(~ Sector, nrow = 4)
```
