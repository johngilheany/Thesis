...
```{r}
#download package
devtools::install_github("nnguyen2496/mscir")
library(mscir)
...
```{r}
data(minvol_weight)
ggplot(data = minvol_weight) + geom_line(mapping = aes(x = Date, y = Weight)) + scale_x_date(labels = date_format("%m-%Y"), date_breaks = "1 year") +  facet_wrap(~ Sector, nrow = 4)
```
